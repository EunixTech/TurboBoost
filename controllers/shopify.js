    ...
    try {
        ...
        if (isNull(connection) || connection.length === 0) { // checking for user exist or not
            ...
            if (userData && Object.keys(userData).length > 0) {
                ...
                userId = newAdmin._id
                //create new user
                createMixpanelEvent("New Trials", {
                    firstName: first_name,
                    lastName: last_name,
                    id: userId,
                    email: email,
...
exports.v1testconnection = async (req, res) => {
    ...
    try {
        ...
        await state.save();
        var uri = `https://${shop}/admin/oauth/authorize?client_id=${SHOPIFY_API_KEY}&scope=${SHOPIFY_API_SCOPES}&state=${state.key}&redirect_uri=${SHOPIFY_API_REDIRECT}`
        res.redirect(uri);
